    ...
    try {
        ...
        for (const data of dataEntries) {
            ...
            const fileWatcher = fs.watch(profile_folder_path, { persistent: true }, async (event, filename) => {
                const fullPath = await path.join(profile_folder_path, filename).replace(/\\/g, '/');
                ...
                if (!isChangeDetected && !filecheckup || !isChangeDetected) {
                    ...
                    if (fs.existsSync(profile_backup_path)) {
                        ...
                        fs.readdir(profile_backup_path, async (err, files) => {
                            ...
                            if (files.length) {
                                ...
                                const data = await ProfileFolderRestore.findOne({
                                    where: {
                                ...
                                if (!data) {
                                    ...
                                    // Send email
                                    const emailSubject = ` Profiles Folder Backup Restored on Machine ${pchostname}`;
                                    const emailText = `
                                                        Hello Team,<br>
                                                        <br>
                                                                A Profiles Folder Backup was restored on ${updateAndLogCurrentDate()} on Machine ${pchostname} due to an anomaly. Details:<br>
                                                        <br>
                                                        - Backup File Path: ${profile_backup_path}<br>
                                                        - Restoration Location: ${profile_folder_path}<br>
                                                        <br>
                                                        Thank you!
                                                        `;
                                    ...
                                    output.on("close", async function () {
                                        ...
                                        fs.rename(zipfolder_path, destinationPath, async (err) => {
                                            if (err) {
                                                ...
                                                // Send email
                                                const emailSubject = `Automatic Backup for Profiles Folder on Machine ${system_name}`;
                                                const emailText = `
                                                        Hello Team,<br>
                                                        <br>
                                                                An automatic backup for the Profiles Folder was performed on ${updateAndLogCurrentDate()} on Machine ${system_name}. Details:<br>
                                                        <br>
                                    ...
                                    if (files.length) {
                                        ...
                                        extractedFiles.forEach(async (file) => {
                                            ...
                                            if (file !== path.basename(zipFilePath)) {
                                                ...
                                                // Send email
                                                const emailSubject = `gvariables File Backup Restored on Machine ${system_name}`;
                                                const emailText = `
                                                        Hello Team,<br>
                                                        <br>
                                                               A gvariables File Backup was restored on ${updateAndLogCurrentDate()} on Machine ${system_name} due to an anomaly.. Details:<br>
                                                        <br>
                                                        - Backup File Path: ${zipFilePath}<br>
                                                        - Restoration Location: ${globalvar_path}<br>
                                                        <br>
                                                        Thank you!
                                                        `;
                                                const isEmailSent = await sendEmail(emailSubject, emailText);
            ...
            for (const item of data) {
                if (item.system_name === pchostname) {
                    ...
                    if (!profileBackupPathExists) {
                        ...
                        isChangeDetected = true;
                        await profilegetbackup(item.system_name, item.profile_path, item.profile_backup_path);
                    }
                    if (profileBackupPathExists) {
                    ...
                    const globalvarbackuppathExists = fs.existsSync(item.globalvar_backup_path);

                    if (!globalvarbackuppathExists) {
                        console.log("Profile backup path does not exist.");
                        isChangeDetected = true;
                        await gvarfilegetbackup(item.system_name, item.globalvar_path, item.globalvar_backup_path);
                    }
                    if (globalvarbackuppathExists) {
                        const files = await util.promisify(fs.readdir)(item.globalvar_backup_path);
                        if (files.length === 0) {
                            console.log("Profile backup folder exists but is empty.");
                            isChangeDetected = true;
                            await gvarfilegetbackup(item.system_name, item.globalvar_path, item.globalvar_backup_path);
                                    ...
                                    fs.rename(zipfolder_path, destinationPath, async (err) => {
                                        if (err) {
                                            ...
                                            if (updatedRows[0] === 0) {
                                                ...
                                                // Send email
                                                const emailSubject = `Profiles Folder Backup Restored on Machine ${system_name}`;
                                                const emailText = `
                                                        Hello Team,<br>
                                                        <br>
                                                            A Profiles Folder Backup was restored on ${updateAndLogCurrentDate()} on Machine ${system_name} due to an anomaly. Details:<br>
                                                        <br>
                                                        - Backup File Path: ${destinationPath} 
                                                        - Restoration Location: ${profile_folder_path} 
                                                        <br>
                                                        Thank you!
                                                        `;
                                                const isEmailSent = await sendEmail(emailSubject, emailText);
                                    ...
                                    if (updatedRows[0] === 0) {
                                        ...
                                        const emailText = `
                                            ...
                                            <br>
                                                 A gvariables File Backup was restored on ${updateAndLogCurrentDate()} on Machine ${system_name} due to an anomaly. Details:<br>
                                            <br>
                                                - Backup File Path: ${zipFilePath}
                                                - Restoration Location: ${globalvar_file_path}
                                            <br>
                                            Thank you!
                                            `;
                                        const isEmailSent = await sendEmail(emailSubject, emailText);
...
const profilegetbackup = async (system_name, profile_path, profile_backup_path) => {
    const zipfolder_path = profile_path + ".zip";

    console.log("System Name:", system_name);
    console.log("Profile Folder Path:", profile_path);
    console.log("Profile Backup Path:", profile_backup_path);
    // profile_backup_path code
    if (fs.existsSync(profile_backup_path)) {
        console.log("exists", profile_backup_path);
        if (fs.existsSync(profile_path)) {
            console.log("exists", profile_path);
            fs.readdir(profile_path, (err, files) => {
                if (err) {
                    console.error("Error:", err);
                    return;
                }
                    ...
                    output.on("close", async function () {
                        ...
                        fs.rename(zipfolder_path, destinationPath, async (err) => {
                            if (err) {
                                ...
                                //     //                     <br>
                                //     //                     - Backup File Path: ${destinationPath} 
                                //     //                     - Restoration Location: ${profile_path} 
                                //     //                     <br>
                                //     //                     Thank you!
                                //     //                     `;
                                //     // const isEmailSent = await sendEmail(emailSubject, emailText);
                                ...
                                //     // }
                                // }
                                // UploadFile(destinationPath, profile_path, system_name);
                            }
                        });
                    });
                }
    ...
    iisChangeDetected = false;
    await ProfileModeCheck();
};
const gvarfilegetbackup = async (system_name, globalvar_path, globalvar_backup_path) => {
    const zipFileName = 'gvariables.zip';
    const zipFilePath = path.join(globalvar_backup_path, zipFileName);
    const filepath = path.join(globalvar_backup_path, path.basename(globalvar_path));
    if (fs.existsSync(globalvar_backup_path)) {
                    ...
                    output.on("close", async function () {
                        ...
                        fs.rename(zipFilePath, destinationPath, async (err) => {
                            if (err) {
                                ...
                                // });
                                // // Send email
                                // const emailSubject = `Automatic Backup for gvariables File on Machine ${system_name}`;
                                // const emailText = `
                                //                         Hello Team,<br>
                                //                         <br>
                                //                                An automatic backup for the gvariables File was performed on ${updateAndLogCurrentDate()} on Machine ${system_name}. Details:<br>
                                //                         <br>
                                //                         -  Backup Folder Path: ${zipFilePath} (Timestamp: ${updateAndLogCurrentDate()}).<br>
                                //                         <br>
                                //                         Thank you!
                                //                         `;
                                // const isEmailSent = await sendEmail(emailSubject, emailText);
